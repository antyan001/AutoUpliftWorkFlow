...
class Node:
    ...
    def debug(self, show_details):
        """Print an ASCII visualization of the tree."""
        lines, _, _, _ = self._debug_aux(show_details, root=True)
        for line in lines:
            print(line)

    def _debug_aux(self, show_details, root=False):
        ...
        if is_leaf:
            ...
            lines = [
                "{} < {:.4f}".format('feat_'+str(self.feature_index), self.threshold)
            ]
        if show_details:
            lines += [
                "uplift = {:.4f}".format(self.uplift),
                "samples = {}".format(self.n_items)
            ]
        ...
        left, n, p, x = self.left._debug_aux(show_details)
        right, m, q, y = self.right._debug_aux(show_details)
